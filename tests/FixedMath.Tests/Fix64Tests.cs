...
namespace FixedMath
{
    public class Fix64Tests
    {
        ...
        public void Abs()
        {
            Assert.Equal(Fix64.MaxValue, MathFix.Abs(Fix64.MinValue));
            var sources = new[] { -1, 0, 1, int.MaxValue };
            var expecteds = new[] { 1, 0, 1, int.MaxValue };
            for (int i = 0; i < sources.Length; ++i)
            {
                var actual = MathFix.Abs((Fix64)sources[i]);
        ...
        public void Log2()
        {
            ...
            for (int j = 0; j < m_testCases.Length; ++j)
            {
                ...
                if (b <= Fix64.Zero)
                {
                    Assert.Throws<ArgumentOutOfRangeException>(() => MathFix.Log2(b));
                }
                else
                {
                    var expected = Math.Log((double)b) / Math.Log(2);
                    var actual = (double)MathFix.Log2(b);
